...
// Import task lists from our clean database file
...
export default function App() {
  ...
  // State to toggle between Volume 1, 2, and 3
    ...
    <div className="min-h-screen bg-thxbai-dark text-white font-sans p-4 md:p-12 relative selection:bg-transparent">
      ...
      <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-12 gap-5">
        ...
        <motion.div whileHover={{ scale: 1.005 }} className="md:col-span-4 glass-card rounded-[2.5rem] p-10 flex flex-col justify-center min-h-[200px]">
          ...
          <p className="text-thxbai-muted text-xs font-bold tracking-[0.3em] uppercase opacity-50">Est. 2026 // Season 2</p>
        </motion.div>

        {/* MC Server Real-time Tracking Panel */}
        <motion.div whileHover={{ scale: 1.005 }} className="md:col-span-8 glass-card rounded-[2.5rem] p-10 flex items-center justify-between overflow-hidden relative">
          <div>
            ...
            <p className="text-thxbai-muted text-lg font-medium italic">Vol. {activeVolume} Operations Active.</p>
          </div>
          <div className="flex flex-col items-end">
            ...
            <span className="flex items-center gap-2 text-[10px] uppercase font-black text-green-500 tracking-[0.2em] mt-2 select-none">
              <span className="w-2 h-2 bg-green-500 rounded-full animate-ping" /> Active Players
            </span>
          </div>
        </motion.div>

        {/* COMPACT VOLUME SELECTOR TAB ROW */}
          ...
          {[1, 2, 3].map((volNum) => (
            <button 
              ...
              onClick={() => {
                ...
                setSelectedFaction(leaderboard[0]?.name || "");
              }}
              className={`px-6 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all duration-200 cursor-pointer ${
                activeVolume === volNum ? "bg-white text-black font-black" : "bg-white/5 border border-white/5 text-thxbai-muted hover:bg-white/10 hover:text-white"
              }`}
            >
              Volume {volNum}
            </button>
        ...
        {/* LIVE EVENT LOGISTICS SUMMARY PANEL */}
        <motion.div className="md:col-span-12 glass-card rounded-[3rem] p-12 min-h-[380px] flex flex-col justify-between border-l-4 border-l-thxbai-accent">
          <div>
            <div className="flex items-center gap-3 mb-6 select-none">
              <span className="px-3 py-1 rounded-full bg-thxbai-accent/20 text-thxbai-accent text-[10px] font-black uppercase tracking-widest border border-thxbai-accent/10">Active Phase</span>
              <span className="text-thxbai-muted text-[10px] font-bold uppercase tracking-widest">Rewards: Discord Nitro</span>
            </div>
            ...
            <p className="text-thxbai-muted text-md max-w-2xl font-medium leading-relaxed mb-6">
              ...
              {activeVolume === 2 && "The advanced phase. Tiered difficulty levels ranging from aesthetics to complex automation finds."}
              {activeVolume === 3 && "The final frontier. Prepare your faction for the supreme task structures coming soon."}
            </p>
          </div>
          <div className="border-t border-white/5 pt-6">
            <span className="block text-[10px] font-black text-thxbai-accent uppercase mb-2 italic underline tracking-wider select-none">Admin Transmission Log</span>
        ...
        {/* FACTION LEADERBOARD GRID PANEL */}
        <motion.div className="md:col-span-12 glass-card rounded-[3rem] p-10 border-t-4 border-t-yellow-500/20 shadow-xl">
          <div className="flex justify-between items-center mb-8">
            <div>
              ...
              <p className="text-thxbai-muted text-xs font-bold mt-1 uppercase tracking-widest opacity-50">Vol. {activeVolume} Global Rankings</p>
            </div>
        ...
        {/* INTERACTIVE FACTION PROGRESS TRACKER CHECKLIST */}
            ...
            <div>
              <div className="flex justify-between items-center mb-6 border-b border-white/5 pb-4">
                <span className="text-xs font-black text-thxbai-muted uppercase tracking-widest">
                  Completed {activeFactionDetails.activeLogs?.length || 0} of {currentTasks.length} Objectives
                </span>
                <span className="text-sm font-black italic text-emerald-400">
                  {currentTasks.length > 0 ? Math.round((activeFactionDetails.activeLogs?.length / currentTasks.length) * 100) : 0}% Complete
                </span>
              </div>
              
              {currentTasks.length === 0 ? (
                <div className="p-12 text-center border border-dashed border-white/10 rounded-2xl">
                  <p className="text-sm text-thxbai-muted italic">Volume 3 Objectives data array initialization pending setup...</p>
                </div>
              ) : (
